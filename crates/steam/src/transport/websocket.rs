...
impl WebSocketTransport {
    pub async fn connect(server: &CmServer) -> Result<Self, Error> {
        let (host, port) = match &server.addr {
            ...
            crate::connection::CmServerAddr::Resolved(addr) => {
        ...
        let (ws, _) = connect_async(&url)
            .await
            .map_err(|e| ConnectionError::Io(std::io::Error::new(std::io::ErrorKind::ConnectionRefused, e)))?;
    ...
    fn send(
        ...
        Box::pin(async move {
            self.sink
                ...
                .send(msg)
                .await
                .map_err(|e| ConnectionError::Io(std::io::Error::new(std::io::ErrorKind::BrokenPipe, e)))?;
            Ok(())
        })
    }
